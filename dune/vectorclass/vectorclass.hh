...
namespace Dune {
  namespace Simd {
    namespace VectorclassImpl {
      ...
      /* true for all boolean vectorclass-types*/
...
#undef DUNE_VECTORCLASS_VECTORCLASS_SIMD_TYPES
    ...
    namespace Overloads {
      ...
      template<class Vec, typename = std::enable_if_t<VectorclassImpl::IsVector<Vec>::value>>
      VectorclassImpl::Proxy<Vec> lane(ADLTag<5>, std::size_t l, Vec &v) {
        return {l, v};
      }
  ...
  template<class Vec>
  struct AutonomousValueType<Simd::VectorclassImpl::Proxy<Vec>> :
    AutonomousValueType<typename Simd::VectorclassImpl::Proxy<Vec>::value_type> {};
...
namespace VCL_NAMESPACE {
  ...
  template<class Vec,
           typename = std::enable_if_t<Dune::Simd::VectorclassImpl::IsVector<Vec>::value>>
...
#endif //DUNE_VECTORCLASS_VECTORCLASS_HH
